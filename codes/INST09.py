...
if __name__ == "__main__":
    ruta_instancia = "../Benchmarks/INST09.txt"
    ...
    datos_INST09 = cargar_instancia_khan(ruta_instancia)
    ...
    consumos_finales = np.dot(mejor_solucion.vector_binario, datos_INST09['r_ij_k'])
    print(f"▶ consumo total por recurso k: {consumos_finales}")
    
    if np.all(consumos_finales <= datos_INST09['b_k']):
    ...
    print(f"▶ total de ítems seleccionados: {total_seleccionados} de {datos_INST09['n']} grupos")
