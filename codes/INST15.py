...
if __name__ == "__main__":
    ruta_instancia = "../Benchmarks/INST15.txt"
    ...
    datos_INST15 = cargar_instancia_khan(ruta_instancia)
    ...
    print(f"▶ benchmark de referencia mabc_2000: .")
    ...
    consumos_finales = np.dot(mejor_solucion.vector_binario, datos_INST15['r_ij_k'])
    print(f"▶ consumo total por recurso k: {consumos_finales}")
    
    if np.all(consumos_finales <= datos_INST15['b_k']):
    ...
    print(f"▶ total de ítems seleccionados: {total_seleccionados} de {datos_INST15['n']} grupos")
